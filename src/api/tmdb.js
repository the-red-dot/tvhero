export default async function handler(req, res) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`TMDb API Error: ${errorData.status_message} (Code: ${errorData.status_code})`);
        res.status(response.status).json({ error: `TMDb API Error: ${errorData.status_message}` });
        return;
      }
